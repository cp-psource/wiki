...
class WikiWidget extends WP_Widget {
		...
		function __construct() {
			...
			$widget_ops = array( 'description' => __('Wiki-Seiten anzeigen', 'wiki') );
	...
	function form($instance) {
		...
		<div style="text-align:left">
			...
			<label for="<?php echo $this->get_field_id('hierarchical'); ?>" style="line-height:35px;display:block;"><?php _e('Ebenene', 'wiki'); ?>:<br />
				...
				<option value="0" <?php if ($options['hierarchical'] == 0){ echo 'selected="selected"'; } ?> ><?php _e('Unlimitiert', 'wiki'); ?></option>

					</select>

				</label>

			<label for="<?php echo $this->get_field_id('order_by'); ?>" style="line-height:35px;display:block;"><?php _e('Ordne nach', 'wiki'); ?>:<br />

					<select id="<?php echo $this->get_field_id('order_by'); ?>" name="<?php echo $this->get_field_name('order_by'); ?>" >

				<option value="menu_order" <?php if ($options['order_by'] == 'menu_order'){ echo 'selected="selected"'; } ?> ><?php _e('Menüreihenfolge / Reihenfolge erstellt', 'wiki'); ?></option>

				<option value="title" <?php if ($options['order_by'] == 'title'){ echo 'selected="selected"'; } ?> ><?php _e('Titel', 'wiki'); ?></option>

				<option value="rand" <?php if ($options['order_by'] == 'rand'){ echo 'selected="selected"'; } ?> ><?php _e('Zufällig', 'wiki'); ?></option>

					</select>

				</label>

			<label for="<?php echo $this->get_field_id('order'); ?>" style="line-height:35px;display:block;"><?php _e('Ordnen', 'wiki'); ?>:<br />

					<select id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>" >

				<option value="ASC" <?php if ($options['order'] == 'ASC'){ echo 'selected="selected"'; } ?> ><?php _e('Aufsteigend', 'wiki'); ?></option>

				<option value="DESC" <?php if ($options['order'] == 'DESC'){ echo 'selected="selected"'; } ?> ><?php _e('Absteigend', 'wiki'); ?></option>

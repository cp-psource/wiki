...
class Wiki_Premium {
	...
	public function register_taxonomies() {
		...
		register_taxonomy('psource_wiki_category', 'psource_wiki', array(
			...
			'labels' => array(
				'name' => __( 'Wiki-Kategorien', 'ps-wiki' ),
				'singular_name' => __( 'Wiki-Kategorie', 'ps-wiki' ),
				'search_items' => __( 'Suche in Wiki-Kategorien', 'ps-wiki' ),
				'all_items' => __( 'Alle Wiki-Kategorien', 'ps-wiki' ),
				'parent_item' => __( 'Übergeordnete Wiki-Kategorie', 'ps-wiki' ),
				'parent_item_colon' => __( 'Übergeordnete Wiki-Kategorie:', 'ps-wiki' ),
				'edit_item' => __( 'Wiki-Kategorie bearbeiten', 'ps-wiki' ),
				'update_item' => __( 'Wiki-Kategorie aktualisieren', 'ps-wiki' ),
				'add_new_item' => __( 'Neue Wiki-Kategorie hinzufügen', 'ps-wiki' ),
				'new_item_name' => __( 'Neuer Name der Wiki-Kategorie', 'ps-wiki' ),
			),
		...
		register_taxonomy('psource_wiki_tag', 'psource_wiki', array(
			...
			'labels' => array(
				'name'			=> __( 'Wiki-Tags', 'ps-wiki' ),
				'singular_name'	=> __( 'Wiki-Tag', 'ps-wiki' ),
				'search_items'	=> __( 'Suche Wiki-Tags', 'ps-wiki' ),
				'popular_items'	=> __( 'Beliebte Wiki-Tags', 'ps-wiki' ),
				'all_items'		=> __( 'Alle Wiki-Tags', 'ps-wiki' ),
				'edit_item'		=> __( 'Wiki-Tag bearbeiten', 'ps-wiki' ),
				'update_item'	=> __( 'Wiki-Tag aktualisieren', 'ps-wiki' ),
				'add_new_item'	=> __( 'Neues Wiki-Tag hinzufügen', 'ps-wiki' ),
				'new_item_name'	=> __( 'Neuer Wiki-Tag-Name', 'ps-wiki' ),
				'separate_items_with_commas'	=> __( 'Trenne Wiki-Tags durch Kommas', 'ps-wiki' ),
				'add_or_remove_items'			=> __( 'Hinzufügen oder Entfernen von Wiki-Tags', 'ps-wiki' ),
				'choose_from_most_used'			=> __( 'Wähle häufig verwendete Wiki-Tags', 'ps-wiki' ),
			),
			'show_admin_column' => true,
		));
	}
...
<tr valign="top">
	<th><label for="psource_wiki-breadcrumbs_in_title"><?php _e('Anzahl der Breadcrumbs, die dem Titel hinzugefügt werden sollen', 'ps-wiki'); ?></label> </th>
...
<tr valign="top">
	<th><label for="psource_wiki-wiki_name"><?php _e('Wie möchtest Du Wikis nennen?', 'ps-wiki'); ?></label> </th>
...
<tr valign="top">
	<th><label for="psource_wiki-sub_wiki_name"><?php _e('Wie möchtest Du Sub Wikis nennen?', 'ps-wiki'); ?></label> </th>
...
<tr valign="top">
	<th><label for="psource_wiki-sub_wiki_order_by"><?php _e('Wie sollen Sub-Wikis bestellt werden?', 'ps-wiki'); ?></label> </th>

	<td>
		<select id="psource_wiki-sub_wiki_order_by" name="wiki[sub_wiki_order_by]" >
			<option value="menu_order" <?php selected($this->wiki->get_setting('sub_wiki_order_by'), 'menu_order'); ?>><?php _e('Menüreihenfolge/Reihenfolge erstellt', 'ps-wiki'); ?></option>
			<option value="title" <?php selected($this->wiki->get_setting('sub_wiki_order_by'), 'title'); ?>><?php _e('Titel', 'ps-wiki'); ?></option>
			<option value="rand" <?php selected($this->wiki->get_setting('sub_wiki_order_by'), 'rand'); ?>><?php _e('Zufällig', 'ps-wiki'); ?></option>
		</select>
	</td>
</tr>

<tr valign="top">
	<th><label for="psource_wiki-sub_wiki_order"><?php _e('Welche Reihenfolge sollten Sub Wikis bestellt werden?', 'ps-wiki'); ?></label> </th>

	<td>
		<select id="psource_wiki-sub_wiki_order" name="wiki[sub_wiki_order]" >
			<option value="ASC" <?php selected($this->wiki->get_setting('sub_wiki_order'), 'ASC'); ?>><?php _e('Aufsteigend', 'ps-wiki'); ?></option>
			<option value="DESC" <?php selected($this->wiki->get_setting('sub_wiki_order'), 'DESC'); ?>><?php _e('Absteigend', 'ps-wiki'); ?></option>
		</select>
	</td>
</tr>

<tr valign="top">
	<th><label><?php _e('Wer kann Wiki-Berechtigungen bearbeiten?', 'ps-wiki'); ?></label> </th>
	...
	public function add_meta_boxes( $post ) {
		if ( $post->post_author == wp_get_current_user()->ID || current_user_can('edit_posts') ) {
			add_meta_box('psource-wiki-privileges', __('Wiki-Berechtigungen', 'ps-wiki'), array(&$this, 'privileges_meta_box'), 'psource_wiki', 'side');
		}
	}
	...
	public function privileges_meta_box( $post, $echo = true ) {
		...
		$privileges = array(
			'anyone' => __('Jeder', 'ps-wiki'),
			'network' => __('Netzwerk Benutzer', 'ps-wiki'),
			'site' => __('Seitenbenutzer', 'ps-wiki'),
			'edit_posts' => __('Benutzer, die Beiträge auf dieser Seite bearbeiten können', 'ps-wiki')
			);

		$content .= '<input type="hidden" name="psource_wiki_privileges_meta" value="1" />';
		$content .= '<div class="alignleft">';
		$content .= '<b>'. __('Bearbeitung zulassen durch', 'ps-wiki').'</b><br/>';
	...
	public function wiki_taxonomies( $echo = true ) {
		...
		$content .= wp_dropdown_categories( array(
						...
						'class' => '',
						'echo' => false,
						'show_option_none' => __( 'Kategorie wählen...', 'ps-wiki')
					) );
		$content .= '</td>';
		$content .= '<td id="wiki-tags-label" style="width: 50px">';
		$content .= '<label for="wiki-tags">'.__('Tags:', 'ps-wiki').'</label>';
		$content .= '</td>';

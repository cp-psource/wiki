...
class Wiki {
	...
	function new_wiki_form() {
		...
		echo '<h3>'.__('BEARBEITEN', 'ps-wiki').'</h3>';
		...
		echo	'<input type="submit" name="save" id="btn_save" value="'.__('Speichern', 'ps-wiki').'" />&nbsp;';
		echo	'<a href="'.get_permalink().'">'.__('Abbrechen', 'ps-wiki').'</a>';
		echo	'</div>';
		echo	'</form>';
		echo	'</div>';
	...
	function get_edit_form($showheader = false) {
		...
		if ( !current_user_can('edit_wiki', $post->ID) && !current_user_can('edit_wikis', $post->ID) && !current_user_can('edit_others_wikis', $post->ID) && !current_user_can('edit_published_wikis', $post->ID) ) {
			return __('Du hast keine Berechtigung zum Anzeigen dieser Seite.', 'ps-wiki');
		}
		...
		if ( 'private' == $edit_post->post_status ) {
				...
				$visibility = 'private';
					$visibility_trans = __('Privat', 'ps-wiki' );
		} elseif ( !empty( $edit_post->post_password ) ) {
				$visibility = 'password';
				$visibility_trans = __('Passwortgeschützt', 'ps-wiki' );
		} else {
				$visibility = 'public';
				$visibility_trans = __('Öffentlich', 'ps-wiki' );
		}
		...
		$return .= '<input type="submit" name="save" id="btn_save" value="'.__('Speichern', 'ps-wiki').'" />&nbsp;';
		$return .= '<a href="'.get_permalink().'">'.__('Abbrechen', 'ps-wiki').'</a>';
		$return .= '</div>';
		$return .= '</form>';
	...
	function tabs() {
		...
		$tabs	 = '<ul class="left">';
		$tabs .= '<li class="'.join(' ', $classes['page']).'" ><a href="' . $permalink . '" >' . __('Wiki-Seite', 'ps-wiki') . '</a></li>';

		if (comments_open()) {
			$tabs .= '<li class="'.join(' ', $classes['discussion']).'" ><a href="' . add_query_arg('action', 'discussion', $permalink) . '">' . __('Diskussion', 'ps-wiki') . '</a></li>';
		}

		$tabs .= '<li class="'.join(' ', $classes['history']).'" ><a href="' . add_query_arg('action', 'history', $permalink) . '">' . __('Verlauf', 'ps-wiki') . '</a></li>';
		$tabs .= '</ul>';
		...
		if ($post && current_user_can($post_type_object->cap->edit_post, $post->ID)) {
			$tabs .= '<ul class="right">';
			$tabs .= '<li class="'.join(' ', $classes['edit']).'" ><a href="' . add_query_arg('action', 'edit', $permalink) . '">' . __('Bearbeiten', 'ps-wiki') . '</a></li>';
			if (is_user_logged_in()) {
    ...
    *                                         Formulartabellenformat.
	...
	function register_post_types() {
		...
		register_post_type('psource_wiki', array(
				'labels' => array(
					'name' 				 => __('Wikis', 'ps-wiki'),
					'singular_name' 	 => __('Wiki', 'ps-wiki'),
					'add_new' 			 => __('Wiki hinzufügen', 'ps-wiki'),
					'add_new_item'  	 => __('Neues Wiki hinzufügen', 'ps-wiki'),
					'edit_item'			 => __('Wiki bearbeiten', 'ps-wiki'),
					'new_item' 			 => __('Neues Wiki', 'ps-wiki'),
					'view_item' 		 => __('Wiki anzeigen', 'ps-wiki'),
					'search_items'  	 => __('Suche Wiki', 'ps-wiki'),
					'not_found' 		 =>	 __('Kein Wiki gefunden', 'ps-wiki'),
					'not_found_in_trash' => __('Im Papierkorb wurden keine Wikis gefunden', 'ps-wiki'),
					'menu_name' 		 => __('Wikis', 'ps-wiki')
				),
	...
	function meta_boxes() {
		...
		if ($post->post_author == $current_user->ID || current_user_can('edit_posts')) {
			add_meta_box('psource-wiki-notifications', __('Wiki-E-Mail-Benachrichtigungen', 'ps-wiki'), array(&$this, 'notifications_meta_box'), 'psource_wiki', 'side');
		}
	}
	...
	function name_save($post_name) {
    	if (isset($_POST['post_type']) && $_POST['post_type'] == 'psource_wiki' && empty($post_name)) {

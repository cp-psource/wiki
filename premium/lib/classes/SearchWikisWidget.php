...
class SearchWikisWidget extends WP_Widget {
    function __construct() {
		$widget_ops = array( 'description' => __('Wiki-Seiten durchsuchen', 'ps-wiki') );
      	$control_ops = array( 'title' => __('Suche Wikis', 'ps-wiki') );

		//parent::WP_Widget( 'psource_search_wikis', __('Suche Wikis', 'wiki'), $widget_ops, $control_ops );
		parent::__construct( 'psource_search_wikis', __('Suche Wikis', 'ps-wiki'), $widget_ops, $control_ops );
    }
    ...
    function widget($args, $instance) {
		...
		$options = $instance;
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Suche Wikis', 'ps-wiki') : $instance['title'], $instance, $this->id_base);
		?>
		...
		<form role="search" method="get" id="searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>" >
			<div><label class="screen-reader-text" for="s"><?php _e('Search for:'); ?></label>
				...
				<input type="submit" id="searchsubmit" value="<?php esc_attr_e('Suche', 'ps-wiki'); ?>" />
			</div>
		</form>
		<br />
    ...
    function update($new_instance, $old_instance) {
        ...
        $new_instance = wp_parse_args( (array) $new_instance, array( 'title' => __('Suche Wikis', 'ps-wiki')) );
        $instance['title'] = strip_tags($new_instance['title']);
        return $instance;
    }

    function form($instance) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => __('Suche Wikis', 'ps-wiki')));
